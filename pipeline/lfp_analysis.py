...
import trodes.read_exported.organize_all_trodes_export as organize_export
import trodes.read_exported.read_trodes_extracted_data_file as read_extract
import pandas as pd
import numpy as np
...
#  need to more modular, power functions (from notebook 2) are automatically
#   called at object creation
#  power, phase, coherence, granger functions depend on each other
#   (add none exceptions)
...
class LFPObject:
    def make_object(self):
        ...
        session_trodes_temp, paths = extract_all_trodes(self.path)
        # call add_video_timestamps
        session_trodes_temp = add_video_timestamps(
            session_trodes_temp,
            self.path
        )
        # call create_metadata_df
        metadata = create_metadata_df(session_trodes_temp, paths)
        # call adjust_first_timestamps
        first_timestamps_out = adjust_first_timestamps(
            metadata,
            self.path,
            self.subject
        )
        metadata, state_df, video_df, final_df, pkl_path = first_timestamps_out
        # assign variables
    ...
    def make_coherence_df(self):
        # call get_coherence
        # lfp_traces_df, resample_rate, time_halfbandwidth_product,
        # time_window_duration, time_window_step
        coherence_df = calculate_coherence(
            self.spike_df, self.RESAMPLE_RATE,
            self.TIME_HALFBANDWIDTH_PRODUCT,
            self.TIME_WINDOW_DURATION,
            self.TIME_WINDOW_STEP
        )
    ...
    def make_filter_bands_df(self):
        ...
        filter_bands_df = calculate_filter_bands(
            ...
            theta_band=self.BAND_FREQ["theta"],
            gamma_band=self.BAND_FREQ["gamma"],
            output_dir=os.getcwd(), output_prefix="test"
        )
        # assign variables
    ...
    def analyze_sleap(self):
        # start_stop_frame_df, plot_output_dir, output_prefix,
        # thorax_index, thorax_plots, save_plots=False
        thorax_index = 1
        analyze_sleap_file(
            start_stop_frame_df=self.sleap_df,
            plot_output_dir="test_outputs/",
            output_prefix="test",
            thorax_index=thorax_index,
            thorax_plots=True, save_plots=False
        )
    ...
    def __init__(self,
        ...
        self.BAND_FREQ = {"theta": (4, 12), "gamma": (30, 51)}

        # TODO: power stuff, save only columns of power, phase,
        # coherence, granger
        self.power_df = None
        self.phase_df = None
        ...
        # RECORDING_EXTENTION, LFP_FREQ_MIN, LFP_FREQ_MAX,
        # ELECTRIC_NOISE_FREQ, LFP_SAMPLING_RATE, EPHYS_SAMPLING_RATE):
...
def helper_find_nearest_indices(array1, array2):
    """
    Finds the indices of the elements in array2 that are nearest to the
    elements in array1.

    This function flattens array1 and for each number in the flattened array,
    finds the index of the
    number in array2 that is nearest to it. The indices are then reshaped to
    match the shape of array1.
    ...
    Returns:
    - numpy.ndarray: An array of the same shape as array1, containing the
    indices of the nearest numbers
...
def helper_generate_pairs(lst):
    ...
    Returns:
    - list: A list of tuples, each containing a unique pair from the input
    list.
    """
    n = len(lst)
...
def helper_update_array_by_mask(array, mask, value=np.nan):
    """
    Update elements of an array based on a mask and replace them with a
    specified value.

    Parameters:
    - array (np.array): The input numpy array whose values are to be updated.
    - mask (np.array): A boolean array with the same shape as `array`.
    Elements of `array` corresponding to True in the mask are replaced.
    - value (scalar, optional): The value to assign to elements of `array`
    where `mask` is True. Defaults to np.nan.

    Returns:
    - np.array: A copy of the input array with updated values where the mask
    is True.
...
def helper_compute_velocity(node_loc, window_size=25, polynomial_order=3):
    ...
    The function utilizes the Savitzky-Golay filter to smooth the data and
    ...
    window_size : int, optional
        ...
        Represents the number of consecutive data points used when smoothing
        the data.
        Default is 25.

    polynomial_order : int, optional
        The order of the polynomial fit to the data within the Savitzky-Golay
        filter window.
        Default is 3.
    ...
    -------
    numpy.ndarray
        The velocity for each frame, calculated from the smoothed x and y
        coordinates.
...
def get_sleap_tracks_from_h5(filename):
    ...
    This function is intended for use with Pandas' apply method on columns
...
def extract_sleap_data(filename):
    """
    Extracts coordinates, names of body parts, and track names from a
    SLEAP file.
    ...
    Example:
    output= extract_sleap_data("path/to/sleap/file.h5")
    locations, node_names, track_names = output
    """
    result = {}
    with h5py.File(filename, "r") as f:
...
def rescale_dimension_in_array(arr, dimension=0, ratio=1):
    """
    Rescale values of a specified dimension in a 3D numpy array for the
    entire array.

    Parameters:
    - arr (numpy.ndarray): A 3D numpy array where the third dimension is
    being rescaled.
    - dimension (int, default=0): Specifies which dimension (0 or 1) of the
    third
...
def convert_mp4(experiment_dir):
    """
    Converts .h264 files to .mp4 files using the bash script convert_mp4.sh
    convert_mp4.sh should exist in the same directory as this script.
    Args:
        experiment_dir (String): Path to the experiment directory containing
        subdirectories with .h264 files.
            For example, if your experiment contains the following
            subdirectories:
...
def extract_all_trodes(input_dir):
    ...
    Returns:
        session_trodes_data (defaultdict): A nested dictionary containing
        the metadata for each session.
    """

    def recursive_dict():
        return defaultdict(recursive_dict)

    session_trodes_data = recursive_dict()
    session_path = {}

    # This loop will process each session directory using the trodes extract
    # functions and store the metadata in a
    # nested dictionary.
    ...
    for session in glob.glob(input_dir):
        try:
            ...
            session_trodes_data[session_basename] = organize_export(session)
            session_path[session_basename] = session
        except Exception as e:
            print("Error processing session: ", session_basename)
            print(e)

    # print(session_trodes_data)
    return session_trodes_data, session_path


def add_video_timestamps(session_trodes_data, directory_path):
    """
    Args:
        session_trodes_data (Nested Dictionary): Generate from
        extract_all_trodes.
        directory_path (String): Path containing the session directories to
        process.

    Returns:
        session_trodes_data (Nested Dictionary): A nested dictionary
        containing the metadata for each session.
    """

    # Loops through each session and video_timestamps file and adds the
    # timestamps to the session_trodes_data
    # dictionary. Timestamp array is generated using the
    # read_trodes_extracted_data_file function from the
    ...
    for session in glob.glob(directory_path):
        try:
            ...
            for video_timestamps in glob.glob(
                ...
                timestamp_array = read_extract(video_timestamps)

                session = session_trodes_data[session_basename]
                session_base = session[session_basename]

                if "video_timestamps" not in session_base:
                    session_base["video_timestamps"] = defaultdict(dict)

                session_base["video_timestamps"][video_basename.split(
                    "."
                    )[-3]] = timestamp_array
        ...
        return session_trodes_data


def create_metadata_df(session_trodes, session_path):
    """

    Args:
        session_trodes (nested dictionary): Generated from
        extract_all_trodes.
        session_path (empty dictionary): {}
        columns_keep (dictionary): Provide a dictionary of the columns to keep
        in the metadata dataframe.

    Returns:
        trodes_metadata_df (pandas dataframe): A dataframe containing the
        metadata for each session.
    """
    trodes_metadata_dict = {
        (i, j, k, y): session_trodes[i][j][k][y]
        for i in session_trodes.keys()
        for j in session_trodes[i].keys()
        for k in session_trodes[i][j].keys()
        for y in session_trodes[i][j][k].keys()
    }

    trodes_metadata_df = pd.DataFrame.from_dict(
        trodes_metadata_dict,
        orient='index'
    )

    trodes_metadata_df = trodes_metadata_df.reset_index()
    trodes_metadata_df = trodes_metadata_df.rename(columns={
        'level_0': 'session_dir',
        'level_1': 'recording',
...
def get_trodes_video_df(trodes_metadata_df):
    """
    Extracts the video data from the trodes_metadata_df and calculates the
    first timestamp for each session.
    Args:
        trodes_metadata_df (pandas dataframe): Generated from
        create_metadata_df.
    Returns:
        trodes_video_df (pandas dataframe): A dataframe containing the video
        data for each session.
    """
    meta_df_dir = trodes_metadata_df["metadata_dir"]
    trodes_video_df = (
        trodes_metadata_df[meta_df_dir == "video_timestamps"]
        .copy()
        .reset_index(drop=True)
    )
    trodes_video_df = (
        trodes_video_df[trodes_video_df["metadata_file"] == "1"].copy()
    )
...
def get_trodes_state_df(trodes_metadata_df):
    """
    Extracts the state data from the trodes_metadata_df and calculates the
    timestamps for each event.
    Args:
        trodes_metadata_df (pandas dataframe): Generated from
        create_metadata_df.
    Returns:
        trodes_state_df (pandas dataframe): A dataframe containing the state
        data for each session.
    """
    trodes_state_df = (
        trodes_metadata_df[trodes_metadata_df["metadata_dir"].isin(["DIO"])]
        .copy()
    )
    trodes_state_df = (
        trodes_metadata_df[trodes_metadata_df["id"].isin([
            "ECU_Din1",
            "ECU_Din2"
            ])].copy()
    )
    ...
    trodes_state_df["event_indexes"] = trodes_state_df.apply(
        lambda x: np.column_stack([
            np.where(x["last_item_data"] == 1)[0],
            np.where(x["last_item_data"] == 1)[0] + 1
            ]), axis=1)

    trodes_state_df["event_indexes"] = trodes_state_df.apply(
        lambda x: x["event_indexes"][
            x["event_indexes"][:, 1] <= x["first_item_data"].shape[0] - 1
        ],
        axis=1
    )
...
def get_trodes_raw_df(trodes_metadata_df):
    """
    Extracts the raw data from the trodes_metadata_df and calculates the first
    timestamp for each session.
    Args:
        trodes_metadata_df (pandas dataframe): Generated from
        create_metadata_df.
    Returns:
        trodes_raw_df (pandas dataframe): A dataframe containing the raw data
        for each session.
    """
    trodes_raw_df = trodes_metadata_df[
        (trodes_metadata_df["metadata_dir"] == "raw") &
        (trodes_metadata_df["metadata_file"] == "timestamps")
    ].copy()
    ...
    trodes_raw_df = (
        trodes_raw_df[trodes_raw_cols].reset_index(drop=True).copy()
    )
...
def make_final_df(trodes_raw_df, trodes_state_df, trodes_video_df):
    """
    Merges the trodes_raw_df and trodes_state_df dataframes and calculates the
    timestamps for each event.
    Args:
    ...
    Returns:
        trodes_final_df (pandas dataframe): A dataframe containing the final
        data for each session.
    """
    ...
    for col in [col for col in trodes_final_df.columns if "timestamps" in col]:
        trodes_final_df[col] = trodes_final_df.apply(
            lambda x: x[col].astype(np.int32) - np.int32(x["first_timestamp"]),
            axis=1
        )
...
def merge_state_video_df(trodes_state_df, trodes_video_df):
    """
    Cleans the trodes_state_df and trodes_video_df dataframes and merges them
    on the session_dir column.
    Args:
        trodes_state_df (pandas dataframe): Generated from get_trodes_state_df.
        trodes_video_df (pandas dataframe): Generated from get_trodes_video_df.
    Returns:
    ...
    trodes_state_df = trodes_state_df[state_cols_keep].drop_duplicates(
        subset=["session_dir", "metadata_file"]
        ).sort_values(
    ...
    first_agg = {
        col: 'first'
        for col in trodes_state_df.columns
        if col not in same_columns + different_columns
    }
    ...
    agg_dict = {**first_agg, **list_agg}

    trodes_state_df = (
        trodes_state_df
        .groupby(same_columns)
        .agg(agg_dict)
        .reset_index()
    )

    trodes_state_df["tone_timestamps"] = (
        trodes_state_df["event_timestamps"].apply(lambda x: x[0])
    )
    trodes_state_df["port_entry_timestamps"] = (
        trodes_state_df["event_timestamps"].apply(lambda x: x[1])
    )

    trodes_state_df["tone_frames"] = (
        trodes_state_df["event_frames"].apply(lambda x: x[0])
    )
    trodes_state_df["port_entry_frames"] = (
        trodes_state_df["event_frames"].apply(lambda x: x[1])
    )
    trodes_state_df = (
        trodes_state_df.drop(
            columns=["event_timestamps", "event_frames"],
            errors="ignore"
        )
    )
    print(trodes_state_df.head())
...
def adjust_first_timestamps(trodes_metadata_df, output_dir, experiment_prefix):
    """
    The function will adjust the first timestamps for each session and create
    a final dataframe containing the
    metadata for each session.
    Args:
        trodes_metadata_df (pandas dataframe): Generated from
        create_metadata_df.
    ...
    Returns:
        ...
        data for each session.
    """
    trodes_metadata_df = add_subjects_metadata(trodes_metadata_df)
    metadata_cols_keep = ['raw', 'DIO', 'video_timestamps']
    trodes_metadata_df = (
        trodes_metadata_df[trodes_metadata_df["metadata_dir"].isin(
            metadata_cols_keep
        )].copy()
    )
    trodes_metadata_df = (
        trodes_metadata_df[~trodes_metadata_df["metadata_file"].str.contains(
            "out"
        )]
    )
    trodes_metadata_df = (
    ...
    recording_first_timestamp = (
        trodes_raw_df.set_index('session_dir')['first_timestamp'].to_dict()
    )
    print(recording_first_timestamp)
    trodes_metadata_df["first_timestamp"] = (
        trodes_metadata_df["session_dir"].map(recording_first_timestamp)
    )
    ...
    # Pickle the final dataframe in the output directory with the
    # experiment prefix.
...
def load_data(channel_map_path, pickle_path, SUBJECT_COL="Subject"):
    ...
    Args:
        ...
        SUBJECT_COL (String): Column name for the subject in the channel
        mapping dataframe.
    Returns:
        CHANNEL_MAPPING_DF (pandas dataframe): A dataframe containing the
        channel mapping data.
        SPIKEGADGETS_EXTRACTED_DF (pandas dataframe): A dataframe containing
        the trodes metadata.
    """
    # Load channel mapping
    CHANNEL_MAPPING_DF = pd.read_excel(channel_map_path)
    CHANNEL_MAPPING_DF = CHANNEL_MAPPING_DF.drop(
        columns=[col for col in CHANNEL_MAPPING_DF.columns if "eib" in col],
        errors="ignore"
    )
...
def extract_lfp_traces(
    ...
    """
    Extracts the LFP traces from the SpikeGadgets recordings using the
    spikeextractors module.
    Args:
        ALL_SESSION_DIR (String): Path to the directory containing the session
        directories.
        ...
        LFP_FREQ_MIN (float): The minimum frequency for the LFP bandpass
        filter.
        LFP_FREQ_MAX (float): The maximum frequency for the LFP bandpass
        filter.
    ...
    Returns:
        recording_name_all_ch_lfp (dictionary): A dictionary containing the
        LFP traces for each recording.
    """
    recording_name_all_ch_lfp = {}
    print("ALL SESSION DIR is " + ALL_SESSION_DIR)
    for session_dir in glob.glob(ALL_SESSION_DIR):
        for recording_path in glob.glob(os.path.join(
            session_dir, RECORDING_EXTENTION
        )):
            try:
                recording_basename = os.path.splitext(os.path.basename(
                    recording_path
                ))[0]
                crrnt_rcrdng = se.read_spikegadgets(
                    recording_path,
                    stream_id=ECU_STREAM_ID
                )
                crrnt_rcrdng = se.read_spikegadgets(
                    recording_path,
                    stream_id=TRODES_STREAM_ID
                )
                print(recording_basename)

                # Preprocessing the LFP
                crrnt_rcrdng = sp.notch_filter(
                    crrnt_rcrdng,
                    freq=ELECTRIC_NOISE_FREQ
                )
                crrnt_rcrdng = sp.bandpass_filter(
                    crrnt_rcrdng,
                    freq_min=LFP_FREQ_MIN,
                    freq_max=LFP_FREQ_MAX
                )
                crrnt_rcrdng = sp.resample(
                    crrnt_rcrdng,
                    resample_rate=LFP_SAMPLING_RATE
                )
                recording_name_all_ch_lfp[recording_basename] = crrnt_rcrdng
            except Exception as error:
                print("An exception occurred:", error)
    print("LENGTH OF RECORDING NAME TO ALL CH LFP")
    print(len(recording_name_all_ch_lfp))
    return recording_name_all_ch_lfp
...
def combine_lfp_traces_and_metadata(
    ...
    Args:
        ...
        ALL_CH_LFP_COL (String): The column name for the LFP traces in the
        SpikeGadgets dataframe.
        SUBJECT_COL (String): Column name for the subject in the channel
        mapping dataframe.
        CURRENT_SUBJECT_COL (String): Column name for the current subject in
        the SpikeGadgets dataframe.
    Returns:
        SPIKEGADGETS_FINAL_DF (pandas dataframe): A dataframe containing the
        final data for each session.
    """
    print("recording name to all channel")
    print(recording_name_all_ch_lfp)
    print(SPIKEGADGETS_EXTRACTED_DF.columns)
    lfp_trace_condition = (SPIKEGADGETS_EXTRACTED_DF["recording"].isin(
        recording_name_all_ch_lfp
    ))
    print(lfp_trace_condition)

    SPIKEGADGETS_LFP_DF = (
        SPIKEGADGETS_EXTRACTED_DF[lfp_trace_condition]
        .copy()
        .reset_index(drop=True)
    )
    print("on line 494")
    SPIKEGADGETS_LFP_DF["all_ch_lfp"] = (
        SPIKEGADGETS_LFP_DF["recording"].map(recording_name_all_ch_lfp)
    )
    print("on line 496")
    SPIKEGADGETS_LFP_DF["LFP_timestamps"] = SPIKEGADGETS_LFP_DF.apply(
        lambda row: np.arange(
            0,
            row["all_ch_lfp"].get_total_samples() * LFP_RESAMPLE_RATIO,
            LFP_RESAMPLE_RATIO,
            dtype=int
        ),
        axis=1
    ...
    print("on line 502")
    SPIKEGADGETS_LFP_DF["all_channels"] = (
        SPIKEGADGETS_LFP_DF["all_ch_lfp"].apply(lambda x: x.get_channel_ids())
    )
    SPIKEGADGETS_LFP_DF["region_channels"] = (
        SPIKEGADGETS_LFP_DF[[
    ...
    SPIKEGADGETS_LFP_DF["region_channels"] = (
        SPIKEGADGETS_LFP_DF["region_channels"].apply(
    ...
    def get_traces_with_progress(row):
        ...
        Returns:
            traces (numpy array): A numpy array containing the LFP traces for
            each region.
        """
    ...
    SPIKEGADGETS_LFP_DF["all_region_lfp_trace"] = SPIKEGADGETS_LFP_DF.apply(
        get_traces_with_progress,
        axis=1
    )
    print("on line 508")

    def create_lfp_trace_dict(row):
        region_keys = [
            "{}_lfp_trace".format(t[0].strip("spike_interface_"))
            for t in row["region_channels"]
        ]
        return dict(zip(region_keys, row["all_region_lfp_trace"]))

    SPIKEGADGETS_LFP_DF["per_region_lfp_trace"] = (
        SPIKEGADGETS_LFP_DF.apply(create_lfp_trace_dict, axis=1)
    )
    SPIKEGADGETS_FINAL_DF = pd.concat(
        [
            SPIKEGADGETS_LFP_DF.copy(),
            SPIKEGADGETS_LFP_DF['per_region_lfp_trace'].apply(pd.Series).copy()
        ],
        axis=1
    )
    print("on line 510")
    SPIKEGADGETS_FINAL_DF = SPIKEGADGETS_FINAL_DF.drop(columns=[
        "all_channels",
    ...
    spk_cols = SPIKEGADGETS_FINAL_DF.columns
    SPIKEGADGETS_FINAL_DF = (
        SPIKEGADGETS_FINAL_DF.drop(
            columns=[
                col for col in spk_cols if "spike_interface" in col
            ],
            errors="ignore"
        )
    )
    SPIKEGADGETS_FINAL_DF = (
        SPIKEGADGETS_FINAL_DF.rename(
            columns={col: col.lower() for col in SPIKEGADGETS_LFP_DF.columns}
        )
    )
    sorted_columns = sorted(spk_cols, key=lambda x: x.split("_")[-1])
...
def preprocess_lfp_data(
    lfp_traces_df,
    voltage_scaling_value,
    zscore_threshold,
    resample_rate
):
    """
    Preprocesses the LFP traces in the input dataframe by calculating the
    modified z-score, root-mean-square, and filtering out outliers.
    Args:
        lfp_traces_df (pandas dataframe): A dataframe containing the LFP
        traces.
    ...
    Returns:
        lfp_traces_df (pandas dataframe): A dataframe containing the
        preprocessed LFP traces.
    """
    ...
    for col in original_trace_columns:
        ...
        lfp_traces_df[updated_column] = (
            lfp_traces_df.apply(
                lambda x: 0.6745*(x[col] - np.median(x[col])) / x[MAD_column],
                axis=1
            )
        )
    ...
    for col in zscore_columns:
        ...
        lfp_traces_df[updated_column] = (
            lfp_traces_df[col].apply(lambda x: np.abs(x) >= zscore_threshold)
        )
    ...
    for col in original_trace_columns:
        ...
        lfp_traces_df[updated_column] = (
            lfp_traces_df.apply(
                lambda x: helper_update_array_by_mask(x[col], x[mask_column]),
                axis=1
            )
        )
...
def calculate_power(
    ...
    Args:
        ...
        LFP traces.
        resample_rate (int): The resample rate for the LFP traces.
        time_halfbandwidth_product (float): The time halfbandwidth product for
        the multitaper method.
        time_window_duration (float): The duration of the time window for the
        multitaper method.
        time_window_step (float): The step size for the time window for the
        multitaper method.
    Returns:
        lfp_traces_df (pandas dataframe): A dataframe containing the LFP
        traces with the power calculated.
    """
    ...
    for col in input_columns:
        ...
        try:
            ...
            lfp_traces_df[connectivity_col] = (
                lfp_traces_df[multitaper_col].apply(
                     lambda x: Connectivity.from_multitaper(x)
                )
            )

            lfp_traces_df[frequencies_col] = (
                lfp_traces_df[connectivity_col].apply(lambda x: x.frequencies)
            )
            lfp_traces_df[power_col] = (
                lfp_traces_df[connectivity_col].apply(
                    lambda x: x.power().squeeze()
                )
            )

            lfp_traces_df[power_col] = (
                lfp_traces_df[power_col].apply(lambda x: x.astype(np.float16))
            )

            lfp_traces_df = (
                lfp_traces_df.drop(
                    columns=[multitaper_col, connectivity_col],
                    errors="ignore"
                )
            )
    ...
    lfp_traces_df["power_timestamps"] = (
        lfp_traces_df["lfp_timestamps"].apply(
            lambda x: x[
                (resample_rate//2):(-resample_rate//2):(resample_rate//2)
            ]
        )
    )
    power_freq_col = [
    ...
    lfp_traces_df["power_calculation_frequencies"] = (
        lfp_traces_df[power_freq_col].copy()
    )
...
def calculate_phase(lfp_traces_df, fs):
    ...
    Returns:
        lfp_traces_df (pandas dataframe): A dataframe containing the LFP
        traces with the phase calculated.
    """
    ...
    order = 4
    RMS_columns = [
        col for col in lfp_traces_df
        if "RMS" in col and "filtered" in col and "all" not in col
    ]
    ...
    for col in band_columns:
        ...
        lfp_traces_df[updated_column] = (
            lfp_traces_df[col].apply(lambda x: np.angle(hilbert(x), deg=False))
        )
...
def calculate_coherence(
    ...
    Returns:
        lfp_traces_df (pandas dataframe): A dataframe containing the LFP
        traces with the coherence calculated.
    """
    ...
    brain_region_pairs = (
        helper_generate_pairs(
            list(
                set([col.split("lfp")[0].strip("_") for col in input_columns])
            )
        )
    )
    ...
    for first_region, second_region in brain_region_pairs:
        for suffix in all_suffixes:
            ...
            pair_base_name = (
                f"{region_1.split('_')[0]}_{region_2.split('_')[0]}_"
                f"{suffix_for_name}"
            )
            ...
            try:
                ...
                coherence_col = (
                    f"{pair_base_name}_"
                    f"coherence_all_frequencies_all_windows"
                )
                ...
                lfp_traces_df[coherence_col] = (
                    lfp_traces_df[coherence_col].apply(
    ...
    lfp_traces_df["coherence_timestamps"] = (
        lfp_traces_df["lfp_timestamps"].apply(
            lambda x: x[
                (resample_rate//2):(-resample_rate//2):(resample_rate//2)
            ]
    ...
    lfp_traces_df["coherence_calculation_frequencies"] = (
        lfp_traces_df[
            [
                col for col in lfp_traces_df.columns
                if "coherence_frequencies" in col
            ][0]
        ].copy()
    )
    lfp_traces_df = (
...
def calculate_granger_causality(
    ...
    """
    Calculates the Granger causality of the LFP traces using the multitaper
    method.
    Args:
        lfp_traces_df (pandas dataframe): A dataframe containing the LFP
        traces.
    ...
    Returns:
        lfp_traces_df (pandas dataframe): A dataframe containing the LFP
        traces with the Granger causality calculated.
    """
    print("calculating granger causality")
    input_columns = [
        col for col in lfp_traces_df.columns
        if "trace" in col or "RMS" in col
    ]
    all_suffixes = set(["_".join(col.split("_")[1:]) for col in input_columns])
    ...
    for first_region, second_region in brain_region_pairs:
        for suffix in all_suffixes:
            ...
            try:
                ...
                granger_1_2_col = (
                    f"{region_1_base_name}_{region_2_base_name}_"
                    f"granger_all_frequencies_all_windows"
                )
                granger_2_1_col = (
                    f"{region_2_base_name}_{region_1_base_name}_"
                    f"granger_all_frequencies_all_windows"
                )
                lfp_traces_df[multitaper_col] = lfp_traces_df.apply(
                    lambda x: Multitaper(
                ...
                lfp_traces_df[granger_1_2_col] = (
                    lfp_traces_df[granger_1_2_col].apply(
                        lambda x: x.astype(np.float16)
                    )
                )
                lfp_traces_df[granger_2_1_col] = (
                    lfp_traces_df[granger_2_1_col].apply(
                        lambda x: x.astype(np.float16)
                    )
                )
    ...
    lfp_traces_df["granger_timestamps"] = (
        lfp_traces_df["lfp_timestamps"].apply(
            lambda x: x[
                (resample_rate//2):(-resample_rate//2):(resample_rate//2)
            ]
    ...
    lfp_traces_df["granger_calculation_frequencies"] = (
        lfp_traces_df[
            [
                col for col in lfp_traces_df.columns
                if "granger_frequencies" in col
            ][0]
        ]
        .copy()
    )
...
def calculate_filter_bands(
    ...
    """
    Filters the LFP spectral data for theta and gamma bands, and saves the
    result to a file.
    ...
    power_columns = [
        col for col in lfp_spectral_df.columns
        if "power" in col and "calculation" not in col and "time" not in col
    ]
    for col in power_columns:
        ...
        lfp_spectral_df[theta_power_col] = (
            lfp_spectral_df.apply(
                lambda x: np.nanmean(
                    x[col][
                        :,
                        (x["power_calculation_frequencies"] >= theta_band[0]) &
                        (x["power_calculation_frequencies"] <= theta_band[1])
                    ],
                    axis=1
                ),
                axis=1
        ...
        lfp_spectral_df[gamma_power_col] = (
            lfp_spectral_df.apply(
                lambda x: np.nanmean(
                    x[col][
                        :,
                        (x["power_calculation_frequencies"] >= gamma_band[0]) &
                        (x["power_calculation_frequencies"] <= gamma_band[1])],
                    axis=1
                ),
                axis=1
            )
    ...
    coherence_columns = [
        col for col in lfp_spectral_df.columns
        if "coherence" in col and not ("calculation" in col or "time" in col)
    ]
    ...
    granger_columns = [
        col for col in lfp_spectral_df.columns if
    ...
    for col in granger_columns:
        ...
        lfp_spectral_df[theta_granger_col] = lfp_spectral_df.apply(
            lambda x: np.nanmean(
                x[col][
                    :,
                    (x["granger_calculation_frequencies"] >= theta_band[0]) &
                    (x["granger_calculation_frequencies"] <= theta_band[1])
                ],
                axis=1
            ),
            axis=1
        ...
        lfp_spectral_df[gamma_granger_col] = lfp_spectral_df.apply(
            lambda x: np.nanmean(
                x[col][
                    :,
                    (x["granger_calculation_frequencies"] >= gamma_band[0]) &
                    (x["granger_calculation_frequencies"] <= gamma_band[1])
                ],
                axis=1
            ),
            axis=1
...
def convert_pixels_cm(start_stop_frame_df, med_pc_width, med_pc_height):
    """
    Converts the pixel values in the input dataframe to centimeters.
    Args:
        start_stop_frame_df (pandas dataframe): A dataframe containing the
        pixel values.
        med_pc_width (float): The width of the video in pixels.
        med_pc_height (float): The height of the video in pixels.
    Returns:
        start_stop_frame_df (pandas dataframe): A dataframe containing the
        pixel values converted to centimeters.
    """
    print("converting pixels to cm")
    print(start_stop_frame_df.columns)
    start_stop_frame_df["bottom_width"] = (
        start_stop_frame_df["corner_coordinate"].apply(
            lambda x: x["box_bottom_right"][0] - x["box_bottom_left"][0]
        )
    )
    start_stop_frame_df["top_width"] = (
        start_stop_frame_df["corner_coordinate"].apply(
            lambda x: x["box_top_right"][0] - x["box_top_left"][0]
        )
    )
    start_stop_frame_df["right_height"] = (
        start_stop_frame_df["corner_coordinate"].apply(
            lambda x: x["box_bottom_right"][1] - x["box_top_right"][1]
        )
    )
    start_stop_frame_df["left_height"] = (
        start_stop_frame_df["corner_coordinate"].apply(
            lambda x: x["box_bottom_left"][1] - x["box_top_left"][1]
        )
    )
    start_stop_frame_df["average_height"] = (
        start_stop_frame_df.apply(
            lambda row: (row["right_height"] + row["left_height"])/2,
            axis=1
        )
    )
    start_stop_frame_df["average_width"] = (
        start_stop_frame_df.apply(
            lambda row: (row["bottom_width"] + row["top_width"])/2,
            axis=1
        )
    )

    start_stop_frame_df["width_ratio"] = (
        med_pc_width / start_stop_frame_df["average_width"]
    )
    start_stop_frame_df["height_ratio"] = (
        med_pc_height / start_stop_frame_df["average_height"]
    )

    start_stop_frame_df["in_video_subjects"] = (
        start_stop_frame_df["in_video_subjects"].apply(lambda x: x.split("_"))
    )
    start_stop_frame_df["subject_tracks"] = (
        start_stop_frame_df.apply(
            lambda x: {
                k: v for k, v in x["subject_tracks"].items()
                if k in x["in_video_subjects"]
            },
            axis=1
        )
    )
    start_stop_frame_df["rescaled_locations"] = start_stop_frame_df.apply(
        lambda x: {
            ...
            for key, value in x["subject_tracks"].items()
        },
        axis=1
    )
    ...
    start_stop_frame_df["rescaled_locations"] = start_stop_frame_df.apply(
        lambda x: {
            key: rescale_dimension_in_array(
                value, dimension=1, ratio=x["height_ratio"]
            )
            for key, value in x["rescaled_locations"].items()
        },
        axis=1
    )

    normalized = pd.json_normalize(start_stop_frame_df["corner_coordinate"])
    start_stop_frame_df = pd.concat(
        [
            start_stop_frame_df.drop(["corner_coordinate"], axis=1),
            normalized
        ],
        axis=1
    )
    ...
    for corner in start_stop_frame_df["corner_parts"].iloc[0]:
        start_stop_frame_df[corner] = start_stop_frame_df.apply(
            lambda x: [
                x[corner][0]*x["width_ratio"],
                x[corner][1]*x["height_ratio"]
            ],
            axis=1
        )
...
def create_individual_pose_tracking_columns(
    start_stop_frame_df
):
    """
    ...
    Returns:
        start_stop_frame_df (pandas dataframe): A dataframe containing the
        pose tracking data with the individual
        columns created.
    """
    start_stop_frame_df = start_stop_frame_df.dropna(subset="current_subject")
    start_stop_frame_df["agent"] = start_stop_frame_df.apply(
        lambda x: list((
            set(x["tracked_subject"]) - set([x["current_subject"]])
        )),
        axis=1
    )
    ...
    start_stop_frame_df["subject_locations"] = start_stop_frame_df.apply(
        lambda x: x["rescaled_locations"][x["current_subject"]],
        axis=1
    )
    start_stop_frame_df["agent_locations"] = start_stop_frame_df.apply(
        lambda x: x["rescaled_locations"].get(
            x["agent"],
            np.nan
        ) if x["agent"] else np.nan,
        axis=1
    )
    # start_stop_frame_df = start_stop_frame_df.drop([
    # "sleap_glob", "subject_index", "subject_tracks",
    # "corner_parts", "corner_coordinate", "bottom_width",
    # "top_width", "right_height", "left_height", "average_height",
    # "average_width", "width_ratio", "height_ratio", 'locations',
    # 'track_names', 'sleap_path', 'corner_path', 'all_sleap_data',
    # 'rescaled_locations'], errors="ignore", axis=1)
...
def calculate_velocity(
    ...
    Returns:
        start_stop_frame_df (pandas dataframe): A dataframe containing the
        pose tracking data with the velocity
        calculated.
    """
    start_stop_frame_df["body_parts"].apply(lambda x: x.index("thorax"))
    start_stop_frame_df["subject_thorax_velocity"] = start_stop_frame_df.apply(
        lambda x: helper_compute_velocity(
            x["subject_locations"][:, x["body_parts"].index("thorax"), :],
            window_size=frame_rate*3
        ) * frame_rate,
        axis=1
    )
    start_stop_frame_df["subject_thorax_velocity"] = (
        start_stop_frame_df["subject_thorax_velocity"].apply(
            lambda x: x.astype(np.float16) if x is not np.nan else np.nan
        )
    )
    start_stop_frame_df["agent_thorax_velocity"] = (
        start_stop_frame_df.apply(
            lambda x: helper_compute_velocity(
                x["agent_locations"][:, x["body_parts"].index("thorax"), :],
                window_size=frame_rate*3
    ...
    start_stop_frame_df["agent_thorax_velocity"] = (
        start_stop_frame_df["agent_thorax_velocity"].apply(
            lambda x: x.astype(np.float16) if x is not np.nan else np.nan)
    )
...
def calculate_distance_reward_port(start_stop_frame_df, thorax_index):
    """
    Calculates the distance between the subject and agent thorax to the reward
    port.
    Args:
        start_stop_frame_df (pandas dataframe): A dataframe containing the
        pose tracking data.
    ...
    Returns:
        start_stop_frame_df (pandas dataframe): A dataframe containing the
        pose tracking data with the distance
    """
    start_stop_frame_df["subject_thorax_reward_port"] = (
        start_stop_frame_df.apply(
            lambda x: np.linalg.norm(
                (
                    x["subject_locations"][:, x["body_parts"].index(
                        "thorax"
                    ), :] -
                    x["reward_port"]
                ),
    ...
    start_stop_frame_df["subject_thorax_reward_port"] = (
        start_stop_frame_df["subject_thorax_reward_port"].apply(
            lambda x: x.astype(np.float16) if x is not np.nan else np.nan)
    )
    start_stop_frame_df["agent_thorax_reward_port"] = (
        start_stop_frame_df.apply(
            lambda x: np.linalg.norm(
                (
                    x["agent_locations"][:, x["body_parts"].index(
                        "thorax"
                    ), :] -
                    x["reward_port"]
                ),
                axis=1
            ) if x["agent_locations"] is not np.nan else np.nan,
            axis=1
        )
    )

    start_stop_frame_df["agent_thorax_reward_port"] = (
        start_stop_frame_df["agent_thorax_reward_port"].apply(
            lambda x: x.astype(np.float16) if x is not np.nan else np.nan
        )
    )
...
def process_sleap_tracks(
    ...
    Returns:
        start_stop_frame_df (pandas dataframe): A dataframe containing the
        pose tracking data with the SLEAP
        tracks processed.
    """
    print(start_stop_frame_df.columns)
    start_stop_frame_df["tracked_subject"] = (
        start_stop_frame_df["tracked_subject"].apply(
    ...
    start_stop_frame_df["sleap_glob"] = (
        start_stop_frame_df["sleap_name"].apply(
            lambda x: glob.glob(os.path.join(sleap_dir, "**", x))
        )
    )

    start_stop_frame_df = (
        start_stop_frame_df[start_stop_frame_df['sleap_glob'].apply(
            lambda x: len(x) >= 1
        )]
    )
    start_stop_frame_df = start_stop_frame_df.reset_index(drop=True)
    start_stop_frame_df["sleap_path"] = (
        start_stop_frame_df["sleap_glob"].apply(lambda x: x[0])
    )
    start_stop_frame_df["all_sleap_data"] = (
        start_stop_frame_df["sleap_path"].apply(
            lambda x: extract_sleap_data(x)
        )
    )
    start_stop_frame_df["body_parts"] = (
        start_stop_frame_df["sleap_path"].apply(
            lambda x: get_node_names_from_sleap(x)
        )
    )
    start_stop_frame_df["locations"] = (
        start_stop_frame_df["all_sleap_data"].apply(
            lambda x: x["locations"]
        )
    )
    start_stop_frame_df["track_names"] = (
        start_stop_frame_df["all_sleap_data"].apply(lambda x: x["track_names"])
    )
    ...
    start_stop_frame_df["subject_index"] = start_stop_frame_df.apply(
        lambda x: {
            k: x["track_names"].index(k) for k in x["tracked_subject"]
            if k in x["track_names"]
        },
        axis=1
    )
    start_stop_frame_df["subject_tracks"] = start_stop_frame_df.apply(
        lambda x: {k: v for k, v in x["subject_index"].items()}, axis=1)
    start_stop_frame_df["subject_tracks"] = start_stop_frame_df.apply(
        lambda x: {
            k: x["locations"][:, :, :, v] for k, v in x[
                "subject_index"
            ].items()
        },
        axis=1
    ...
    start_stop_frame_df["corner_path"] = (
        start_stop_frame_df["sleap_path"].apply(
            lambda x: x.replace(
                "id_corrected.h5",
                "corner.h5"
    ...
    start_stop_frame_df["corner_parts"] = (
        start_stop_frame_df["corner_path"].apply(
            lambda x: get_node_names_from_sleap(x)
        )
    )
    start_stop_frame_df = (
        start_stop_frame_df[start_stop_frame_df["corner_parts"].apply(
            lambda x: "reward_port" in x
        )]
    )
    start_stop_frame_df["corner_coordinate"] = (
        start_stop_frame_df["corner_path"].apply(
            lambda x: get_sleap_tracks_from_h5(x)
        )
    )
    start_stop_frame_df["corner_coordinate"] = start_stop_frame_df.apply(
        lambda x: {
            part: x["corner_coordinate"][:, index, :, :]
    ...
    start_stop_frame_df["corner_coordinate"] = start_stop_frame_df.apply(
        lambda x: {
            k: v[~np.isnan(v)][:2] for k, v in x["corner_coordinate"].items()
        },
        axis=1
    )
...
def preprocess_start_stop_frame_data(start_stop_frame_df, sleap_dir):
    ...
    Args:
        start_stop_frame_df (pandas dataframe): A dataframe containing the
        start/stop frame data.
        sleap_dir (str): The directory containing the SLEAP data.
    Returns:
        start_stop_frame_df (pandas dataframe): A dataframe containing the
        start/stop frame data preprocessed.
    """
    ...
    start_stop_frame_df["start_frame"] = (
        start_stop_frame_df["start_frame"].astype(int)
    )
    start_stop_frame_df["stop_frame"] = (
        start_stop_frame_df["stop_frame"].astype(int)
    )
    start_stop_frame_df = start_stop_frame_df.drop(
        columns=["file_path", "notes"],
        errors="ignore"
    )
...
def combine_with_lfp(start_stop_frame_df, lfp_spectral_df):
    ...
    Args:
        start_stop_frame_df (pandas dataframe): A dataframe containing the
        start/stop frame data.
        lfp_spectral_df (pandas dataframe): A dataframe containing the
        LFP data.
    Returns:
        lfp_and_sleap (pandas dataframe): A dataframe containing the
        start/stop frame data combined with the LFP data.
    """
...
def process_sleap_data(sleap_dir,
    ...
    Args:
        ...
        start_stop_frame_df (pandas dataframe): A dataframe containing the
        start/stop frame data.
        lfp_spectral_df (pandas dataframe): A dataframe containing the LFP
        data.
        thorax_index (int): The index of the thorax in the body parts list.
        output_prefix (str): Prefix for the output files.
    Returns:
    ...
    start_stop_frame_df = convert_pixels_cm(
        start_stop_frame_df,
        med_pc_width,
        med_pc_height
    )
    ...
    start_stop_frame_df = calculate_distance_reward_port(
        start_stop_frame_df,
        thorax_index
    )
    ...
    start_stop_frame_df.to_pickle(
        os.path.join(output_dir, full_lfp_traces_pkl)
    )
...
def analyze_sleap_file(
    ...
    for FILE_INDEX in range(len(start_stop_frame_df)):
        ...
        with h5py.File(
            ...
            current_subject = (
                start_stop_frame_df["current_subject"].iloc[FILE_INDEX]
            )
            locations = (
                start_stop_frame_df["rescaled_locations"]
                .iloc[FILE_INDEX][current_subject]
            )
    ...
    if thorax_plots:
        ...
        if save_plots:
            plt.savefig(
                os.path.join(
                    plot_output_dir, f"{output_prefix}_thorax_locations.png"
                )
            )
        ...
        if save_plots:
            plt.savefig(
                os.path.join(
                    plot_output_dir, f"{output_prefix}_thorax_tracks.png"
                )
            )
...
def read_phy_data(all_phy_dir):
    ...
    """
    recording_cluster_info = {}
    recording_spike_clusters = {}
    recording_spike_times = {}
    ...
    for recording_dir in all_phy_dir:
        try:
            ...
            recording_cluster_info[recording_basename] = pd.read_csv(
                file_path,
                sep="\t"
            )
            ...
            recording_spike_clusters[recording_basename] = np.load(file_path)

            # Read spike times
            file_path = os.path.join(recording_dir, "phy", "spike_times.npy")
            recording_spike_times[recording_basename] = np.load(file_path)
        except Exception as e:
            print(e)
    output = (
        recording_cluster_info,
...
def filter_good_units(recording_cluster_info):
    """
    Filter good units from cluster information.
    """
    recording_cluster_info_df = pd.concat(
        recording_cluster_info,
        names=['recording_name']
    ).reset_index(level=1, drop=True).reset_index()

    good_unit_cluster_info_df = recording_cluster_info_df[
        recording_cluster_info_df["group"] == "good"].reset_index(drop=True)
    recording_good_unit_ids = good_unit_cluster_info_df.groupby(
        'recording_name'
    )['cluster_id'].apply(list).to_dict()

    return recording_good_unit_ids


def combine_spike_data(
    recording_spike_clusters,
    recording_spike_times,
    recording_good_unit_ids,
    sampling_rate
):
    """
    ...
    for recording_dir in recording_spike_clusters.keys():
        try:
            ...
            spike_clusters = recording_spike_clusters[recording_basename]
            spike_times = recording_spike_times[recording_basename]

            spike_df = pd.DataFrame({
                'spike_clusters': spike_clusters,
                'spike_times': spike_times.T[0]
            })
            ...
            merged_df["current_isi"] = (
                merged_df["timestamp_isi"] / sampling_rate
            )

            if not merged_df.empty:
                recording_spike_df[recording_basename] = merged_df
        except Exception as e:
            print(e)

    all_spike_time_df = pd.concat(recording_spike_df.values())
    all_spike_time_df = (
        all_spike_time_df[all_spike_time_df["cluster_id"].isin(
            recording_good_unit_ids[recording_basename]
        )].reset_index(drop=True)
    )
...
def group_neurons_by_recording(all_spike_time_df, max_spikes=None):
    ...
    """
    grouped_df = all_spike_time_df.groupby(
        ['spike_clusters', 'recording_name']
    )["spike_times"].apply(
        lambda x: np.array(x)
    ).reset_index()
    grouped_df = grouped_df.sort_values(
        by=['recording_name', 'spike_clusters']
    ).reset_index(drop=True)
    ...
    grouped_df["spike_times"] = grouped_df["spike_times"].apply(
        lambda x: np.concatenate(
            [x, np.full([max_number_of_spikes - x.shape[0]], np.nan)]
        )
    )

    grouped_df = grouped_df.groupby('recording_name').agg(
        {
            'spike_clusters': lambda x: list(x),
            'spike_times': lambda x: np.array(list(x))
        }).reset_index()
...
def calculate_firing_rates(
    ...
    lfp_spectral_df["neuron_average_fr"] = lfp_spectral_df.apply(
        lambda x: np.array([neuron.spikes.calculate_rolling_avg_firing_rate(
            np.array(times[~np.isnan(times)]),
            stop_time=x["last_timestamp"] - x["first_timestamp"],
            window_size=spike_window,
            slide=spike_window
        )[0] for times in x["spike_times"]]), axis=1)

    lfp_spectral_df["neuron_average_timestamps"] = lfp_spectral_df.apply(
        lambda x: neuron.spikes.calculate_rolling_avg_firing_rate(
            x["spike_times"][0][~np.isnan(x["spike_times"][0])],
            stop_time=x["last_timestamp"] - x["first_timestamp"],
            window_size=spike_window,
            slide=spike_window)[1],
        axis=1
    )

    lfp_spectral_df["neuron_average_fr"] = lfp_spectral_df.apply(
        lambda x: x["neuron_average_fr"] * spike_window,
        axis=1
    )
...
def add_spike_phy(
    ...
    Returns:
        lfp_spectral_df (pandas dataframe): A dataframe containing the LFP
        data with the spike data added.
        grouped_df (pandas dataframe): A dataframe containing the grouped
        neurons.
        all_spike_time_df (pandas dataframe): A dataframe containing all the
        spike times.
    """
    # Read Phy curation file with read function
    recording_cluster_info, recording_spike_clusters, recording_spike_times = (
        read_phy_data(phy_curation_path)
    )
    recording_good_unit_ids = filter_good_units(recording_cluster_info)
    ...
    # save the results
    lfp_spectral_df.to_pickle(os.path.join(
        output_dir,
        f"{output_prefix}_lfp_spectral_df.pkl"
    ))

    grouped_df.to_pickle(os.path.join(
        output_dir,
        f"{output_prefix}_grouped_df.pkl"
    ))

    all_spike_time_df.to_pickle(
        os.path.join(output_dir, f"{output_prefix}_all_spike_time_df.pkl")
    )
...
def main_test_only():
    input_dir = (
        "/Volumes/chaitra/reward_competition_extension/data/standard/"
        "2023_06_*/*.rec"
    )
    ...
    sleap_path = (
        "/Volumes/chaitra/reward_competition_extension/data/proc/sleap/"
    )
    event_path = (
        "/Volumes/chaitra/reward_competition_extension/data/proc/events.xlsx"
    )
    # phy_path = "/Volumes/chaitra/reward_competition_extension/data/phy/"
    # labels_path = (
    # "/Volumes/chaitra/reward_competition_extension/data/labels.xlsx"
    # )
    convert_mp4(experiment_dir)
    paths = {}
    session_trodes_temp, paths = extract_all_trodes(input_dir)
    # session_trodes_temp = add_video_timestamps(
    # session_trodes_temp, input_dir
    # )
    # metadata = create_metadata_df(session_trodes_temp, paths)
    # adj_first_tmstmps = adjust_first_timestamps(
    # metadata,
    # output_dir,
    # experiment_prefix
    # )
    # metadata, state_df, video_df, final_df, pkl_path = adj_first_tmstmps

    print("output from obj creation")
    CHANNEL_MAPPING_DF, SPIKE_DF = load_data(
        channel_map_path=channel_map_path,
        pickle_path="test_outputs/power_df.pkl"
    )
    calculate_filter_bands(
        SPIKE_DF,
        (4, 12),
        (30, 50),
        output_dir,
...
# lfp = LFPObject(path=input_dir, channel_map_path=channel_map_path,
# events_path="test.xlsx", subject="1.4")
"""
